...
const CameraCheckModal = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
      ...
      <button
        onClick={() => setView('fire')}
        className="flex flex-col items-center justify-center bg-red-50 border-2 border-red-200 p-4 rounded-xl hover:bg-red-100 transition shadow-sm h-32"
      >
        <div className="bg-red-600 text-white p-3 rounded-full mb-2">
          <Flame size={24} />
        </div>
        <span className="font-semibold text-red-900">Fire Hazard</span>
      </button>
      ...
      >
        ...
        <span className="font-semibold text-amber-800">Stray Animals</span>
      </button>
    </div>
