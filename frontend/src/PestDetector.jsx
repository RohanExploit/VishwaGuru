...
const PestDetector = ({ onBack }) => {
  ...
  return (
    <div className="flex flex-col h-full p-4 max-w-md mx-auto">
      ...
      <h2 className="text-2xl font-bold mb-4 text-amber-800">Pest Infestation Detector</h2>
      <p className="text-gray-600 mb-4 text-sm">Detect rats, cockroaches, or unsanitary conditions.</p>
      ...
      <div className="flex justify-center gap-4 mt-auto mb-4">
        {!imgSrc ? (
          <button
            ...
            className={`bg-amber-600 text-white px-6 py-3 rounded-full font-semibold shadow-md hover:bg-amber-700 transition w-full ${cameraError ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            Capture Photo
          </button>
        ) : (
