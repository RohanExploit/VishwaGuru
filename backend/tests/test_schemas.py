...
def test_action_plan():
    plan = ActionPlan(
        whatsapp="Test message", email_subject="Subject", email_body="Body", x_post="Post"
    )
...
def test_issue_create_request():
    request = IssueCreateRequest(
        description="Test issue", category=IssueCategory.ROAD, latitude=12.34, longitude=56.78
    )
...
def test_issue_status_update_request():
    request = IssueStatusUpdateRequest(reference_id="ref123", status=IssueStatus.RESOLVED)
...
def test_push_subscription_request():
    request = PushSubscriptionRequest(endpoint="https://example.com", p256dh="key", auth="secret")
...
def test_urgency_analysis_request():
    request = UrgencyAnalysisRequest(description="Urgent issue", category=IssueCategory.ROAD)
