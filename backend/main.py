...
from hf_service import detect_vandalism_clip, detect_flooding_clip, detect_infrastructure_clip, detect_traffic_violation_clip
...
async def detect_traffic_endpoint(image: UploadFile = File(...)):
    ...
    # Run detection (async)
    try:
        detections = await detect_traffic_violation_clip(pil_image)
        return {"detections": detections}
    except Exception as e:
