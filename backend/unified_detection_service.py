...
class UnifiedDetectionService:
    ...
    async def _check_hf_available(self) -> bool:
        ...
        try:
            # A token is required to call the hosted inference API. This used to
            # read the variable and then set _hf_available = True regardless, so
            # an unconfigured deployment reported the hosted backend as
            # available and never fell back to the local model -- every request
            # failed at call time instead of routing around the gap.
            token = os.environ.get("HF_TOKEN") or os.environ.get("HUGGINGFACE_HUB_TOKEN")
            self._hf_available = bool(token)
            if not self._hf_available:
                logger.warning("HF_TOKEN is not set; the hosted inference backend is unavailable.")
            return self._hf_available
        except Exception:
            logger.exception("Failed to determine Hugging Face availability")
    ...
    async def get_status(self) -> dict:
        ...
        if local_available:
            try:
                ...
                logger.debug("Local backend detail lookup failed", exc_info=True)
