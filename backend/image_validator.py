...
def validate_image_file(file_bytes: bytes) -> tuple[Image.Image, str]:
    ...
    try:
        ...
        raise ImageValidationError(f"Invalid or corrupted image file: {str(e)}") from e
    ...
    try:
        ...
        raise ImageValidationError(f"Image file is corrupted or invalid: {str(e)}") from e
    ...
    logger.info(
        f"Image validated successfully: format={image.format}, size={width}x{height}, file_size={file_size / 1024:.2f}KB"
    )
...
async def validate_uploaded_image(file_obj) -> tuple[Image.Image, str]:
    ...
    try:
        ...
        raise ImageValidationError(f"Failed to process uploaded image: {str(e)}") from e
