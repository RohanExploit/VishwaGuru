...
def test_cache_invalidation_behavior():
    ...
    with (
        patch.object(recent_issues_cache, "invalidate") as mock_invalidate,
        ...
        with (
            patch("backend.main.run_in_threadpool") as mock_threadpool,
            patch("backend.main.get_ai_services") as mock_get_ai,
            patch("backend.main.validate_uploaded_file"),
        ):  # Patch validation
            # Mock AI services
            mock_ai_services = MagicMock()
            mock_ai_services.action_plan_service.generate_action_plan = AsyncMock(
                return_value={"whatsapp": "msg"}
            )
            ...
            mock_saved_issue.action_plan = {
                "whatsapp": "msg"
            }  # Dict, as expected by the optimization logic
            ...
            def side_effect(func, *args, **kwargs):
                ...
                if getattr(func, "__name__", "") == "save_issue_db":
                    ...
                    issue.created_at = datetime.datetime.now(datetime.UTC)
